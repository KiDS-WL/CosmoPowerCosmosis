...
def setup(options):
    ...
    config['lin_matter_power_cp'] = cp.cosmopower_NN(restore=True, 
                            restore_filename=os.path.join(path_2_trained_emulator+'/log10_ratio_lin_matter_power_emulator_mead2020_feedback'))

    config['nonlin_matter_power_cp'] = cp.cosmopower_NN(restore=True, 
                            restore_filename=os.path.join(path_2_trained_emulator+'/log10_ratio_nonlin_matter_power_emulator_mead2020_feedback'))
...
def get_cosmopower_inputs(block, z, nz):
    ...
    params_nonlin = {
        ...
        'omch2':     [block[cosmo, 'omch2']]*nz,
        'z':             z,
        'log_T_AGN':     [block.get_double(names.halo_model_parameters, 'log_T_AGN', default=7.8)]*nz
    }
