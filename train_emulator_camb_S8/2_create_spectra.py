...
def spectra_generation_mead2020_feedback(params):
    ...
    if(Path('powerspectra_new/'+version+'_mead2020_feedback_spectra_'+str(int(params[0]))+'.npz').exists()):
        return 0
    ...
    # redshifts = np.arange(0,6,0.5).tolist()
    # redshifts.append(params[6])
    if(params[6]==0.0):
        redshifts = [0.0]
    else:
    ...
    np.savez('powerspectra_new/'+version+'_mead2020_feedback_spectra_'+str(int(params[0])),Plin=Plin,Pnonlin=Pnonlin)
...
for i in tqdm(range(len(params_all['omch2']))):
    if(Path('powerspectra_new/train_mead2020_feedback_spectra_'+str(i)+'.npz').exists()):
        spectra=np.load('powerspectra_new/train_mead2020_feedback_spectra_'+str(i)+'.npz')
...
spectra_generation_mead2020_feedback(params=[-99,np.mean(params_all['obh2']),np.mean(params_all['omch2']),np.mean(params_all['h']),np.mean(params_all['n_s']),np.mean(params_all['S8']),np.mean(params_all['z']),np.mean(params_all['log_T_AGN'])])
spectra=np.load('powerspectra_new/train_mead2020_feedback_spectra_'+str(-99)+'.npz')
...
# missing_indices = []
# for i in tqdm(range(len(params_all['omch2']))):
#     if(Path('powerspectra_new/test_mead2020_feedback_spectra_'+str(i)+'.npz').exists()):
#         spectra=np.load('powerspectra_new/test_mead2020_feedback_spectra_'+str(i)+'.npz')
#         P_lin.append(spectra['Plin'])
#         P_nonlin.append(spectra['Pnonlin'])
#         available_indices.append(i)
#     else:
