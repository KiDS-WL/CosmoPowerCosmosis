...
def spectra_generation_mead2020_feedback(params):
    ...
    try:
        ...
        np.savez('powerspectra/'+version+'_mead2020_feedback_spectra_'+str(int(params[0])),Plin=Plin,Pnonlin=Pnonlin,boost=boost)
...
def spectra_generation_mead(params):
    ...
    cp = camb.set_params(ombh2 = params[1],
                         ...
                         tau=0.079,
                         TCMB=2.726, YHe=0.25,
                         HMCode_A_baryon=params[7],
                         HMCode_eta_baryon=params[8],
                         redshifts=redshifts,
                         verbose=False)
    ...
    try:
        ...
        np.savez('powerspectra/'+version+'_mead_spectra_'+str(int(params[0])),Plin=Plin,Pnonlin=Pnonlin,boost=boost)
...
params_nonlinear = np.load('outputs/train_mead2020_feedback_parameter_nonlinear.npz')
version='train'

for i in range(45):
    spectra_i = np.arange(i*10000,(i+1)*10000).astype(np.int64)
    arg = zip(np.array([spectra_i,params_linear['obh2'][spectra_i],params_linear['omch2'][spectra_i],params_linear['h'][spectra_i],params_linear['n_s'][spectra_i],params_linear['ln10^{10}A_s'][spectra_i],params_linear['z'][spectra_i],params_nonlinear['log_T_AGN'][spectra_i]]).T)
...
np.savez('outputs/non_linear_matter_mead2020_feedback', modes = k, features = P_nonlin)
...
np.savez('outputs/log10_non_linear_matter_mead2020_feedback', modes = k, features = np.log10(P_nonlin))
...
spectra_generation_mead2020_feedback(params=[-99,np.mean(params_linear['obh2']),np.mean(params_linear['omch2']),np.mean(params_linear['h']),np.mean(params_linear['n_s']),np.mean(params_linear['ln10^{10}A_s']),np.mean(params_linear['z']),np.mean(params_nonlinear['log_T_AGN'])])
...
np.savez('outputs/center_non_linear_boost_mead2020_feedback', modes = k, features = spectra['boost'])


params_linear = np.load('outputs/test_parameter_linear.npz')
params_nonlinear = np.load('outputs/test_mead2020_feedback_parameter_nonlinear.npz')
version='test'
...
np.savez('outputs/non_linear_matter_test_mead2020_feedback', modes = k, features = P_nonlin)
...
# params_linear = np.load('outputs/train_parameter_linear.npz')
# params_boost = np.load('outputs/train_mead_parameter_boost.npz')
# version='train'

# for i in range(9):
#     spectra_i = np.arange(i*20000,(i+1)*20000).astype(np.int64)
#     arg = zip(np.array([spectra_i,params_linear['obh2'][spectra_i],params_linear['omch2'][spectra_i],params_linear['h'][spectra_i],params_linear['n_s'][spectra_i],params_linear['ln10^{10}A_s'][spectra_i],params_linear['z'][spectra_i],params_boost['cmin'][spectra_i],params_boost['eta0'][spectra_i]]).T)
#     pool = multiprocessing.Pool(processes=200)
#     pool.starmap(spectra_generation_mead, arg)
#     pool.close()
...
# nl_boost = []
# for i in tqdm(range(len(params_linear['omch2']))):
#     spectra=np.load('powerspectra/train_mead_spectra_'+str(i)+'.npz')
#     P_lin.append(spectra['Plin'])
#     nl_boost.append(spectra['boost'])

# np.savez('outputs/linear_matter_mead', modes = k, features = P_lin)
# np.savez('outputs/non_linear_boost_mead', modes = k, features = nl_boost)

# np.savez('outputs/log10_linear_matter_mead', modes = k, features = np.log10(P_lin))
# np.savez('outputs/log10_non_linear_boost_mead', modes = k, features = np.log10(nl_boost))




# params_linear = np.load('outputs/test_parameter_linear.npz')
# params_boost = np.load('outputs/test_mead_parameter_boost.npz')
# version='test'
...
# nl_boost = []
# for i in tqdm(range(len(params_linear['omch2']))):
#     spectra=np.load('powerspectra/test_mead_spectra_'+str(i)+'.npz')
#     P_lin.append(spectra['Plin'])
#     nl_boost.append(spectra['boost'])

# np.savez('outputs/linear_matter_test_mead', modes = k, features = P_lin)
# np.savez('outputs/non_linear_boost_test_mead', modes = k, features = nl_boost)